...
function loadPosts(): GalleryPost[] {
  try {
    ...
    // Migration: add missing fields for older entries
...
export const galleryService = {
  ...
  /** Public view: featured first, then by order */
  ...
  add(imageUrl: string, title: string, description?: string, tags?: string[]): GalleryPost {
  ...
  update(id: string, data: Partial<Pick<GalleryPost, 'title' | 'description' | 'tags' | 'imageUrl' | 'featured'>>): void {
    const posts = loadPosts();
