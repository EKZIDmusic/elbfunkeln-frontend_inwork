-- ===============================================
-- Elbfunkeln E-Commerce Database Schema (FIXED)
-- Konsistente UUID-Generierung mit gen_random_uuid()
-- ===============================================

-- Enable UUID extension (for compatibility, though gen_random_uuid() is built-in)
...
CREATE TRIGGER update_contact_inquiries_updated_at BEFORE UPDATE ON contact_inquiries FOR EACH ROW EXECUTE FUNCTION update_updated_at_updated();
...
-- ===============================================
-- Sample Data für Elbfunkeln
-- ===============================================

-- Sample categories
INSERT INTO categories (name, slug, description, is_active, sort_order) VALUES
('Ringe', 'ringe', 'Handgefertigte Drahtschmuck-Ringe aus feinstem Silber- und Golddraht', true, 1),
('Ohrringe', 'ohrringe', 'Elegante Drahtschmuck-Ohrringe in verschiedenen Designs', true, 2),
('Armbänder', 'armbaender', 'Zarte Drahtschmuck-Armbänder für jeden Anlass', true, 3),
('Ketten', 'ketten', 'Kunstvolle Halsketten aus feinem Draht gearbeitet', true, 4)
ON CONFLICT (slug) DO NOTHING;

-- Sample products
INSERT INTO products (name, slug, description, short_description, sku, price, stock_quantity, is_active, is_featured) VALUES
('Eleganter Silberdraht Ring', 'eleganter-silberdraht-ring', 
 'Handgefertigter Ring aus feinstem Silberdraht mit zeitlosem Design. Jeder Ring ist ein Unikat und wird mit viel Liebe zum Detail gefertigt. Perfekt für jeden Anlass und als besonderes Geschenk.', 
 'Zeitloser Ring für jeden Anlass', 'ELB-RING-001', 49.99, 10, true, true),
('Zarte Draht-Ohrringe', 'zarte-draht-ohrringe', 
 'Filigrane Ohrringe aus Draht, die mit ihrer eleganten Form jeden Look vervollständigen. Leicht zu tragen und perfekt für sowohl Alltag als auch besondere Anlässe.', 
 'Filigrane Eleganz für jeden Tag', 'ELB-OHRRINGE-001', 34.99, 15, true, true),
('Armband Silberdraht Vintage', 'armband-silberdraht-vintage', 
 'Kunstvolles Armband im Vintage-Stil, handgearbeitet aus feinstem Silberdraht. Das Design verbindet klassische Eleganz mit modernem Charme.', 
 'Vintage-Charme am Handgelenk', 'ELB-ARMBAND-001', 59.99, 8, true, true),
('Minimalistic Gold Wire Necklace', 'minimalistic-gold-wire-necklace',
 'Eine zarte Halskette aus Golddraht mit minimalistischem Design. Perfekt zum Layering oder als dezentes Statement-Piece.',
 'Minimalistisch & elegant', 'ELB-KETTE-001', 79.99, 12, true, false)
ON CONFLICT (slug) DO NOTHING;
...
DO $$
BEGIN
  RAISE NOTICE '🎉 Elbfunkeln E-Commerce Schema erfolgreich erstellt (FIXED VERSION)!';
  RAISE NOTICE '📊 19 Tabellen mit konsistenten UUID-Spalten angelegt';
  RAISE NOTICE '🔐 Row Level Security aktiviert für alle Tabellen';
  RAISE NOTICE '⚡ Performance-Indizes erstellt';
  RAISE NOTICE '🎯 UUID-Konsistenz: Alle Tabellen nutzen gen_random_uuid()';
